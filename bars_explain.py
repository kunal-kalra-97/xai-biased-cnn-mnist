...
def lime_maps(model, x, tg, grid, N, seed=0):
    seg, n_seg = grid_segments(G=grid)
    out = []
    for i in range(len(x)):
        g = torch.Generator().manual_seed(seed + i)
        out.append(lime_image(model, x[i:i + 1], int(tg[i]), seg, n_seg, N=N, gen=g).relu())
    return torch.stack(out).float()


def run(ckpt_dir, grid=7, N=800, n_eval=None):
    ...
    for v in VARIANTS:
        ...
        results = {
            ...
            f"LIME grid-{grid}": lime_maps(model, x, tg, grid, N),
        }
    ...
    _visualize(ckpt_dir, ev, figs, grid)


def _visualize(ckpt_dir, ev, figs, grid, n=5, seed=1):
    import random
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    run(a.ckpt_dir, grid=a.grid, N=a.n, n_eval=a.n_eval)